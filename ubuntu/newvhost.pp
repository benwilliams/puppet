...
define newvhost($user, $homedir) {
  ...
  exec {"mysql": command => "/usr/bin/mysql < /tmp/mysql.sql", require => File["/tmp/mysql.sql"] }

  file {"/tmp/mysql.sql": content => template("mysql.sql.erb")}

  exec {"/usr/sbin/a2ensite $servername.conf": require => File["/etc/apache2/sites-available/$servername.conf"] }
